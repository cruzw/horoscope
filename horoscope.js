...
exports.getSign = function(month, day) {
  ...
  let sign;
	...
	if (day < 1 || day > 31) throw new Error("check that day isn't zero, negative, or greater than 31")
	let months = {
		1: "January",
		2: "February",
		3: "March",
	...
	let monthFunctions = {
        ...
        return sign;
			...
			May: (day) => {
				(day <= 20) ?
				sign = "Taurus": sign = "Gemini"
        return sign;
			},
			June: (day) => {
				(day <= 20) ?
				sign = "Gemini": sign = "Cancer"
        return sign;
			},
			July: (day) => {
				(day <= 22) ?
        ...
        return sign;
			...
			September: (day) => {
				(day <= 22) ?
				sign = "Virgo": sign = "Libra"
        return sign;
			},
			October: (day) => {
				(day <= 22) ?
				sign = "Libra": sign = "Scorpio"
        return sign;
			},
			November: (day) => {
				(day <= 21) ?
				sign = "Scorpio": sign = "Sagittarius"
        return sign;
			},
			December: (day) => {
				(day <= 21) ?
