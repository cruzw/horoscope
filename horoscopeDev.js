...
let getSign = (month, day) => {
	if (typeof month !== 'number' || typeof day !== 'number') {
		throw new Error('HOROSCOPE.JS - getSign: month/day need to be numbers')
	}
	if (month < 1 || month > 12) {
		throw new Error('HOROSCOPE.JS - getSign: month needs to be between greater than 1, less than 12')
	}
	if (day < 1 || day > 31) {
		throw new Error('HOROSCOPE.JS - getSign: day cant be zero, negative, or greater than than 31')
	}
...
let getZodiac = (year) => {
	if (!year || typeof year !== 'number') {
		throw new Error('HOROSCOPE.JS - getZodiac: year needs to be a number')
	}
	if (year.toString().length !== 4) {
		throw new Error('HOROSCOPE.JS - getZodiac: year needs to be four digits long')
	}
